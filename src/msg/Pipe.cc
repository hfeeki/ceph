...
int Pipe::accept()
{
 ...
 open:
  ...
  if (reply_tag == CEPH_MSGR_TAG_SEQ) {
    ...
    discard_requeued_sent(newly_acked_seq);
  }
...
void Pipe::discard_requeued_sent(uint64_t seq)
{
  ldout(msgr->cct, 10) << "discard_requeued_sent " << seq << dendl;
  list<Message*>& rq = out_q[CEPH_MSG_PRIO_HIGHEST];
  while (!rq.empty()) {
    Message *m = rq.front();
    if (m->get_seq() == 0 || m->get_seq() > seq)
      break;
    ldout(msgr->cct,10) << "discard_requeued_sent " << *m << " for resend seq " << out_seq
